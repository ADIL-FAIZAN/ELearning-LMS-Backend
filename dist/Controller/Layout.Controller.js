...
router.post("/create-layout", auth_1.UpdateAccessToken, isUserAuthenticated, isAdmin, async (req, res, next) => {
    try {
...
router.put("/edit-layout", auth_1.UpdateAccessToken, isUserAuthenticated, isAdmin, async (req, res, next) => {
    try {
        const { type } = req.body;
        if (type === "Banner") {
...
router.get("/get-layout/:type", async (req, res, next) => {
    try {
