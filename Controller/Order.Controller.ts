...
const stripe = require("stripe")(process.env.STRIPE_SECRET_KEY);
 ...
 // Create Course
 router.post("/create-order",UpdateAccessToken,isUserAuthenticated,async (req: Request, res: Response, next: NextFunction) => {
    ...
    const UserCourseExist = userCoursesList.some((e: any) => { if (e.courseId === courseId.toString()){ return true;  } else { return false;} });
    ...
    try {  
      ...
      const UserDb = await User.findById((req as any).user._id);
      UserDb.courses = [...UserDb.courses, {courseId:course?._id}];

        //Redis
      await UserDb.save();
      await redis.set(UserDb._id.toString(), JSON.stringify(UserDb) as any);
  ...
  router.get("/get-all-orders",UpdateAccessToken,isUserAuthenticated,isAdmin,async (req: Request, res: Response, next: NextFunction) => {
